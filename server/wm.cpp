  ...
  if ((WM_MsgPort = Smsgport::Create(2, "WM", 0, 0, 0, WManagerH)) &&
    ...
    if (RegisterExt(WM, MsgPort, WM_MsgPort)) {
      ...
      if ((MapQueue = Smsgport::Create(11, "WM MapQueue", 0, 0, 0, (void (*)(Tmsgport))NoOp))) {
        ...
        if (InitRC()) {
          /* Nemesis: ensure libtw socket is up so external clients (twclutter,
           * twterm, twattach) can attach. Upstream Twin only auto-loads this
           * when running --nohw; we want it unconditionally because the WM
           * pipeline needs a way for clients to spawn windows. */
          if (!DlLoad(SocketSo)) {
            log(WARNING) << "twin: WM: libtw socket module failed to load; "
                            "external Twin clients will not be able to attach.\n";
          }
          /* Best-effort: failure here does not fail server startup --
           * built-in WM remains the sole consumer of map events. */
          WMSockInit();
          return true;
        } else {
