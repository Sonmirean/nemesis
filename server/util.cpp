...
#include "util.h"
#include "wmsock.h" // Nemesis: WMSockShutdown()
...
/* unlink /tmp/.Twin<TWDISPLAY> (and the Nemesis side-channel that lives next to it) */
void QuitTWDisplay(void) {
  /* Nemesis: tear down the WM side-channel first so an attached external WM
   * sees the connection close before its parent libtw socket disappears. */
  WMSockShutdown();
  unlink(fullTWD);
}
