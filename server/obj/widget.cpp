...
void Swidget::UnMap() {
  ...
  if (!w || IS_SCREEN(w)) {
    ...
    if (IS_SCREEN(parent)) {
      /* Mirror to attached external WM (no-op if none). Pair to the map
       * mirror in Swidget::Map(); the internal protocol has no msg_unmap,
       * so it is synthesized here. Built-in WM stays authoritative. */
      WMSockSendUnmap(w, (Tscreen)parent);
    ...
    /* Map mirror already shipped when MapQueueMsg was queued; mirror unmap
     * here so the external WM stays in lock-step. */
    WMSockSendUnmap(w, w->MapQueueMsg->Event.EventMap.Screen);
    w->MapQueueMsg->Delete();
    w->MapQueueMsg = (Tmsg)0;
  }
}
